...
import numpy as np
a=np.array(np.arange(18).reshape(2,3,3))
a.sum(axis=0)                  
a.sum(axis=1)                  
a.sum(axis=2)
#结合布尔索引
k = np.random.randn(50)
np.sum(k > 0)                  
#以下用法与sum（）相似
...
#ptp（）返回沿某轴axis方向上的最大值 - 最小区 差值，
#即 maximum - minimum 的值形成的数组
a = np.arange(9).reshape((3,3))
b = np.ptp(a, axis=0)
c = np.ptp(a, axis=1)
