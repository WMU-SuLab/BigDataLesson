...
arr.transpose((1,0,2))# m[y][x][z] = k[x][y][z]
...
arr.swapaxes(0,1) # 将第一个轴和第二个轴交换 m[y][x][z] = k[x][y][z]
...
Pythonnumpy.delete此函数返回从输入数组中删除指定子数组的新数组。 与insert()函数的情况一样，如果未提供轴参数，则输入数组将展开。 该函数接受以下参数：Numpy.delete(arr, obj, axis)
Python其中：arr：输入数组obj：可以被切片，整数或者整数数组，表明要从输入数组删除的子数组axis：沿着它删除给定子数组的轴，如果未提供，则输入数组会被展开例子import numpy as np
a = np.arange(12).reshape(3,4)
